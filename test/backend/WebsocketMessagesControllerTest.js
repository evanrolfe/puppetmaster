describe('Requests', () => {
  ...
  describe('index', () => {
    it('returns the websocket_messages', async () => {
      ...
      expect(result.result.body[0].body).to.eql('Hello, how are you?');
      expect(result.result.body[0].direction).to.eql('incoming');
      expect(result.result.body[0].request_id).to.eql(1);
      expect(result.result.body[0].url).to.eql('ws://localhost:3002/');

      expect(result.result.body[1].body).to.eql(
        'Hi, Im fine thanks, and yourself?'
      );
      expect(result.result.body[1].direction).to.eql('outgoing');
